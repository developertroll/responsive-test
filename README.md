...
```
2024-03-07
컨디션이 여전히 나빠 작업을 많이 못했음.
그룹챗 컴포넌트 완성, 이제 그룹챗에 들어갈 데이터를 임의로 만들던 받아오던 해야하는데 컨디션 문제로 mongoDB 연결 관련해서 이것 저것 시도해보는데 안되는중
mongoDB에서 값을 받아오는 것에 대해 좀 더 알아보고 다음 작업때 api로 사용할 수 있게 해결을 볼 예정
현재는 nextjs에서 route.js로 지정하면 해당 경로를 api로 인식한다는 것을 확인함
```

```
2024-03-08
컨디션이 좀 회복되서 작업좀 했음.
MongoDB 연결 성공하고 아직 꾸미진 않았지만 그룹방 생성 페이지 및 DB 연결을 진행했는데 문제가 발생함.
현재 그룹방 생성은 되는데 아무런 값이 들어가지 않은 빈 데이터가 생성되고 있어 이 문제를 해결해야함.
현재 확인한 사항: req.body 찍어봤을때 readableStream이 나오고 req.body.title 식으로 찍어보면 undefined가 나오며, route.js에서도 찍어보면 undefined가 전달되고 있어서 그런것으로 보임

6:19 PM 설마 해서 Node.js에서 readableStream을 해석하는 stream, promise와 유사한 기능을 수행하는 utill을 구글링 끝에 찾아서 적용해본 결과 놀랍게도 적용됨. 일단 되는건 확인했지만 해당 코드를 이해하고 쓴건 아니라서 다음 작업때 다시 이 코드를 읽어보고 어떤 식인지 이해해야함
```
